  ...
  $(document).on('click', $('.items-producto'), function(ev){
    ...
    $.getJSON('src/productos.json', function(data){ //GET THE JSON AND data IS THE DATA FROM THE JSON
      for (var i = 0; i < data.productos.length; i++) { //LOOK FOR ALL THE DATA ON THE JSON
        if(id == data.productos[i].id){ //IF THE OPTION SELECTED EQUAL TO ID FROM THE ELEMENT
          $('#desc').text(data.productos[i].Caracteristicas); //FILL THE H1 WITH Caracteristicas, IF YOU WANNA CHANGE THE DOM, YOU JUST CHANGE THE ID
          $('#img_lista').attr('src', data.productos[i].imagen) //FILL THE IMG WITH imagen
          $('#test_select').val(id);
        }
      }
    });
